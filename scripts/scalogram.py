...
def plot_spectrogram(ax, f, t, power, wavelet=None, vmin=None, vmax=None, cmap_name='jet'):
    """
    Plots the scalogram of the LFP signal using the power of the STFT or wavelet transform.
...
def plot_wavelet_stacked(t, lfp, dt, config, scale_low=1, scale_high=200, vmin=None, vmax=None):
    """
    Plots stacked wavelet scalograms based on the given configuration.
    ...
    # Loop through each configuration to generate a scalogram
    for i, (freq_range, wavelet, num_scale) in enumerate(zip(config["ranges"], config["wavelets"], config["num_scales"])):
        ...
        ax.set_ylim(freq_range)  # Set the frequency range for this scalogram
...
def plot_wavelet_stacked_paramsets(paramsets, lfp_pkl_file='lfp.pkl',
    ...
    """
    Plots stacked wavelet scalograms for different paramsets with a consistent wavelet and frequency config.
...
def plot_stft_stacked(t, lfp, dt, config, vmin=None, vmax=None):
    """
    Plots stacked STFT scalograms based on the given configuration.
