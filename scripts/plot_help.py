...
def subplots_mctc_scalogram(paramset, params_short=True, lfp_pkl_file='lfp.pkl',
                          wavelet='cmor1.5-1.0', num_scale=50, freq_range=(10, 200),
                           scale_low=10, scale_high=2000, bp_order=6, vmin=None, vmax=None):
    """
    Visualizes spike data, LFP signals, and a wavelet scalogram.
    """
    ...
    # 3 subplots: spikes, raw LFP, scalogram
    ...
    axs[0].set_xticks(np.arange(min(t), max(t), 200.0), fontsize=36)
    ...
    axs[0].set_xlabel('Simulation Time [ms]', fontsize=40)
    ...
    axs[2].set_xlabel('Simulation Time [ms]', fontsize=40)
    axs[2].set_ylabel('Frequency [Hz]', fontsize=40)
    #axs[2].set_title(f"Wavelet: {wavelet}, Scales: {num_scale}, Freq range: {freq_range}, BP Order: {bp_order}", fontsize=20)
    axs[2].tick_params(labelsize=36)

    # Add colorbar
    cbar = plt.colorbar(contour, ax=axs[2], pad=0.02)
    cbar.set_label('Wavelet Power ($V^2/Hz$)', fontsize=40)
    ...
    filename = os.path.join(save_path, f"mctc_scalogram_{paramset}.png")
...
def subplots_spikes(paramset, params_short=True):
    ...
    min_t = min(t)
    max_t = max(t)
    ax[0].set_xticks(np.arange(min_t, max_t, 50.0), fontsize=18)
    ...
    ax[0].set_xlabel('Simulation Time [ms]', fontsize=20)
    ...
    ax[1].set_xticks(np.arange(min_t, max_t, 50.0), fontsize=16)
    ax[1].set_title('TC Instantaneous Firing Rate [Hz]', fontsize=20)
    ...
    ax[2].set_xticks(np.arange(min_t, max_t, 50.0), fontsize=16)
    ax[2].set_title('MC Instantaneous Firing Rate [Hz]', fontsize=20)
    ...
    ax[3].set_xticks(np.arange(min_t, max_t, 50.0), fontsize=16)
    ax[3].set_title('GC Instantaneous Firing Rate [Hz]', fontsize=20)
    #plot_spikes_hist(ax[3], bincenters, rates, col='orange', linewidth=2)

    ax[3].set_xlabel('Simulation Time [ms]', fontsize=20)
    ...
    save_path = os.path.join(base_dir, "plots")
    os.makedirs(os.path.dirname(save_path), exist_ok=True)  # Ensure the folder exists
    filename = os.path.join(save_path, f"spikesraster_{paramset}.png")
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"Saved to: {os.path.abspath(filename)}")
