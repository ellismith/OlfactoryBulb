...
def get_csd(signal1, signal2):
    # CSD calculates how correlated a signal 1 is in relation to another signal 2
    results_dir, paramset_dir, fig_dir = get_dirs(paramset)
    with open(os.path.join(paramset_dir, 'params.yml'), 'rb') as f:
        params_dict = yaml.load(f, Loader=yaml.FullLoader)
...
def plot_lfp_power_fft(t_lfp, lfp, paramset):
    results_dir, paramset_dir, fig_dir = get_dirs(paramset)
    with open(os.path.join(paramset_dir, 'params.yml'), 'rb') as f:
        params_dict = yaml.load(f, Loader=yaml.FullLoader)
...
def plot_powers_across_paramsets(paramsets, freq_ranges):
    ...
    for idx, (f_min, f_max) in enumerate(freq_ranges):
        ...
        #colors = np.random.rand(len(paramsets), 3)  # Generate random RGB colors

        axs[idx].bar(sorted_paramsets, sorted_powers, color='gray')
        axs[idx].set_ylabel("Power in %i to %i Hz range" % (f_min, f_max), fontsize=fontsize)
        axs[idx].tick_params(axis='x', rotation=45, labelsize=fontsize, align='right')
...
def plot_power_hfo(paramsets):
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('Heatmap of Values at XYZ Locations')
