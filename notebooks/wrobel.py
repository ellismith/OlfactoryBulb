...
def spectr(HFO, freq_lim, ch_list, Fs, vmax=1e-3, ss=0, sts=500):
    ...
    print("HFO[ch_list[0]]:", HFO[ch_list[0]])
    freq, time_spec, spec_mtrx = spectrogram(HFO[ch_list[0]], Fs, nperseg=int(window * Fs))
    ...
    freq, time_spec, spec_mtrx = spectrogram(HFO[ch_list[1]], Fs, nperseg=int(window * Fs), noverlap=0)
    ...
    # Save the figure
    if saveDir_rat:
        print("Saving figure to:", os.path.join(saveDir_rat, 'spectrogram.png'))  # Debug print
        if not os.path.exists(saveDir_rat):  # Check if the directory exists, if not, create it
            os.makedirs(saveDir_rat)
        fig2.savefig(os.path.join(saveDir_rat, 'spectrogram.png'))

def spectr_all(HFO, freq_lim, ch_list, Fs, vmax=1e-3, ss=0, sts=500):
    num_channels = len(ch_list)
    fig2 = py.figure(figsize=(8, num_channels * 2), dpi=150)
    grid = py.GridSpec(num_channels * 3 + 1, 12, hspace=0.5, wspace=0.5)
    colors = cm.get_cmap('Blues', 200)
    for i, ch_index in enumerate(ch_list):
        ax = fig2.add_subplot(grid[i * 3:i * 3 + 3, :], aspect='auto')
        freq, time_spec, spec_mtrx = spectrogram(HFO[ch_index], Fs, nperseg=int(window * Fs))
        ind = np.where(freq < freq_lim)[-1][-1]
        im = ax.pcolormesh(time_spec, freq[:ind], spec_mtrx[:ind], cmap=colors, vmax=vmax)
        py.ylabel('Frequency [Hz]', fontsize=9)
        py.title('ch: ' + ch_pos[ch_index], fontsize=9)
        py.axvline(ss)
        py.axvline(sts)
        py.axvline(ss2, color='r')
        py.axvline(sts2, color='r')
    ...
    # Save the figure
    if saveDir_rat:
        print("Saving figure to:", os.path.join(saveDir_rat, 'spectrogram_all.png'))  # Debug print
        if not os.path.exists(saveDir_rat):  # Check if the directory exists, if not, create it
            os.makedirs(saveDir_rat)
        fig2.savefig(os.path.join(saveDir_rat, 'spectrogram_all.png'))
...
def choosen_freq(HFO, ch_list, Fs, vmax=1e-4, save=1, part='', window=60, low_freq=100, top_freq=180, name_npy='', save_npy=1):
    ...
    freq, time_spec, spec_mtrx = spectrogram(HFO[ch_list[0]], Fs, nperseg=window * Fs, noverlap=0)
    print(freq, time_spec)
    ...
    py.subplot(211)
    py.title(part)
    freq, time_spec, spec_mtrx = spectrogram(HFO[ch_list[1]], Fs, nperseg=window * Fs, noverlap=0)
    
    cp = np.sum(spec_mtrx[ind_h2:ind_h1], axis=0)
    df = freq[list(np.argmax(spec_mtrx[ind_h2:ind_h1], axis=0) + ind_h2)]
    podf = np.max(spec_mtrx[ind_h2:ind_h1], axis=0)
...
condition = 'SAL'  # MODIFY: Set your condition
