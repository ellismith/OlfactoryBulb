...
class OlfactoryBulb:
    ...
    def get_constant_spike_train(self, start_time, duration, frequency, jitter):
        """
        ...
        :param jitter: The maximum amount of random jitter (in ms) added to each spike time
        :return: A numpy array of spike times in chronological order
        """
        
        # Calculate the inter-spike interval in ms
        inter_spike_interval = 1000.0 / frequency  
        ...
        if jitter > 0:
            times += np.random.uniform(-jitter, jitter, size=times.shape)
    ...
    def assign_centrif_ranks(self, centrif_inputsegs):
        ...
        :param centrif_inputsegs: List of GC segment identifiers (e.g., ['GC1[0].soma', 'GC2[0].dend[3]', ...])
        :return: List of tuples containing (seg_address, single_rank_gid, single_rank_address)
        """
        h = self.h
        ...
        for seg in centrif_inputsegs:
            cell = seg.split('.')[0]  # Extract cell name
            rank_cell = self.bn_server.rank_section_name(cell)  # Check if it's on this rank
    ...
    def add_centrifugal_inputs(self, centrif_inputsegs, t):
        ...
        :param centrif_inputsegs: List of GC segment identifiers
        :param t: Onset time for the centrifugal input
        """
        h = self.h
    ...
    def load_cells(self, cell_type):
        ...
        for ri, root in enumerate(group_dict['roots']):
            ...
            # self.cells_raw: {'MC': ['M', 'C', '1', '[', '2', ']', '.', 's', 'o', 'm', 'a']}
